...
NChars <- length(random_chars)
print(NChars)
print(random_chars)
write.csv(data.frame(random_chars), paste0("../trees/seed_",s,"/random_chars.csv"))
...
case='RANDOM_P_FOREST_yr_1981_2000__2001_2020_num_iter_9_boost_1' 
...
write.csv(data.frame(splitting_chars), paste0("../trees/seed_",s,"/splitting_chars.csv"))

first_split_var = c(1:NChars)-1
second_split_var = c(1:NChars)-1
...
no_H1, 
abs_normalize, weighted_loss, lambda_mean, lambda_cov, lambda_mean_factor, lambda_cov_factor, early_stop, stop_threshold, lambda_ridge, a1, a2, list_K)

pred1 = predict(fit1, X_test, R_test, months_test, portfolio_weight_test)

save(fit1,  file = paste0("../trees/seed_",s,"/random_train.RData"))
save(pred1, file = paste0("../trees/seed_",s,"/random_test.RData"))
