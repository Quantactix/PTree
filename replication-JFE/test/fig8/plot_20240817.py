...
def myplot(date, name, my_ylim, n_tick, add_legend=True):
    ...
    plt.savefig('./output/'+name+'_'+date+'_avg20_rpf_NChar10_py.pdf', format='pdf', bbox_inches='tight')
    plt.close()

date = '20240817'
myplot(date, 'mn', 1.0, 6, add_legend=False)
myplot(date, 'sm', 1.5, 6, add_legend=False)
myplot(date, 'sn', 1.5, 6, add_legend=False)
myplot(date, 'sr', 5.0, 6, add_legend=False)
