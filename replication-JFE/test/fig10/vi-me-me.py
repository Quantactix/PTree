...
loss_base = 1.2259728937277161  # the baseline is the Sharpe ratio of Figure 9: (a) ME Baseline P-Tree.
...
find_numbers(case='./baseline-4-ME-ME-invest-1025/char_10',b=1)

def evaluate_a_char(i):
    case = "./baseline-4-ME-ME-invest-1025/char_%s"%i
...
s = pd.DataFrame(index=all_chars[1:])
s['bar']=loss_char
s=s.sort_values('bar')
s['t'] = 1
...
plt.ylim((-0.1,5.0))
plt.hlines(y=loss_base,xmin=-1, xmax=60,color='red')
plt.hlines(y=0, xmin=-1, xmax=60, color='black')
plt.savefig("bar_1ME-2ME-3Char-INVEST-raw-nobench.pdf", bbox_inches = 'tight')
# plt.show()
plt.close()
