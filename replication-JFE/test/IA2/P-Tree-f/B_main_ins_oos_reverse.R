...
tmp = data[,c('gvkey', 'permno', 'sic', 'ret', 'exchcd', 'shrcd', 'date', 'ffi49',
              ...
              'rank_pctacc', 'rank_chtx', # 70
              
              'log_me', # 71
              
              'MKTRF', 'SMB', 'HML', 'RMW', 'CMA', 'MOM', # 77,  FF5 
              ...
              'x_dy', 'x_lev', 'x_ep', 'x_ni', # 97, x_{t-1}  # 87
               
              'xret')] # 98, excess return  # 88
...
data2 <- data[(data[,c('date')]>=oos_start) & (data[,c('date')]<=oos_split), ]
...
ff_test <- ff[row.names(ff)>=oos_start,]
ff_test <- ff_test[row.names(ff_test)<=oos_split,]
