...
df_rppca = read.csv("../../data/RP-PCA/rppca_ins_2001_2020.csv",header = FALSE); colnames(df_rppca) = c(rppca)

df_ipca = read.csv("../../data/IPCA/ipca_ins_2001_2020.csv"); colnames(df_ipca) = c("date", ipca)
df_ipca$date = seq(as.Date(start), as.Date(end),"months")

df_pt =  read.csv("../P-Tree-c/yr_2001_2020__1981_2000_num_iter_9_boost_20_bf_train.csv"); colnames(df_pt) = c("date", pt_factor)
...
unisort <- unisort[c(241:480),]
bisort <- bisort[c(241:480),]
mebm25 <- mebm25[c(241:480),]
ind49 <- ind49[c(241:480),]

## these pt$x data need hand copy paste, which needs to be automated for publication.
pt1 <- read.csv("tmp/P-Tree-c/PT1.csv");colnames(pt1)[1] <- "date";pt1 <- data.frame(pt1, row.names = 1)
pt5 <- read.csv("tmp/P-Tree-c/PT_top5.csv");colnames(pt5)[1] <- "date";pt5 <- data.frame(pt5, row.names = 1)
pt10 <- read.csv("tmp/P-Tree-c/PT_top10.csv");colnames(pt10)[1] <- "date";pt10 <- data.frame(pt10, row.names = 1)
pt20 <- read.csv("tmp/P-Tree-c/PT_top20.csv");colnames(pt20)[1] <- "date";pt20 <- data.frame(pt20, row.names = 1)
...
port_rtn_list = c("pt1","pt5","pt10","pt15","pt20","unisort","bisort","mebm25","ind49")
...
write.csv(data.frame(port_r2), paste0(output_path, 'Portfolio_R2_', case, '-2.csv'))
