...
df_rppca = read.csv("../../data/RP-PCA/rppca_1981_2020.csv",header = FALSE); colnames(df_rppca) = c(rppca)

df_ipca = read.csv("../../data/IPCA/ipca_1981_2020.csv"); colnames(df_ipca) = c("date", ipca)
df_ipca$date = seq(as.Date(start), as.Date(end),"months")

df_pt =  read.csv("../P-Tree-a/yr_1981_2020_num_iter_9_boost_20_bf_train.csv"); colnames(df_pt) = c("date", pt_factor)
...
# data_pt_1_plusMKT = df_pt[,pt_factor_1_plusMKT]
...
pt1 <- read.csv("tmp/P-Tree-a/PT1.csv");colnames(pt1)[1] <- "date";pt1 <- data.frame(pt1, row.names = 1)
pt5 <- read.csv("tmp/P-Tree-a/PT_top5.csv");colnames(pt5)[1] <- "date";pt5 <- data.frame(pt5, row.names = 1)
pt10 <- read.csv("tmp/P-Tree-a/PT_top10.csv");colnames(pt10)[1] <- "date";pt10 <- data.frame(pt10, row.names = 1)
pt20 <- read.csv("tmp/P-Tree-a/PT_top20.csv");colnames(pt20)[1] <- "date";pt20 <- data.frame(pt20, row.names = 1)
...
port_rtn_list = c("pt1","pt5","pt10","pt20","unisort","bisort","mebm25","ind49")
# port_rtn_list = c("mebm25")
