...
RP = pd.read_csv("../../data/RP-PCA/rppca_ins_1981_2000.csv", header=None)
RP.index = pd.date_range(start.strftime("%Y-%m-%d"), split.strftime("%Y-%m-%d"), freq='M')
RP_ins = RP.loc[start.strftime("%Y"):split.strftime("%Y")]
print(RP_ins)

IP = pd.read_csv("../../data/IPCA/ipca_ins_1981_2000.csv", index_col=0)
