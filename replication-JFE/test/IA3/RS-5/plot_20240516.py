...
def myplot(date, name, my_ylim, n_tick, add_legend=True):
    ...
    fig, (ax1, ax3) = plt.subplots(1, 2, figsize=(8, 6), sharey=True, gridspec_kw={'width_ratios': [1, 1]})
    ...
    if add_legend:
        ...
        fig.legend(handles, labels, loc='center right', bbox_to_anchor=(0.9, 0.4))
    else:
        0
    ...
    fig.text(0.5, 0.04, 'c = P/T', ha='center')  # Adjust vertical position with the second parameter if needed

    plt.savefig('./output/'+name+'_'+date+'_avg20_random_N5.py.pdf', format='pdf', bbox_inches='tight')

    plt.close()

date = '20240516'
myplot(date, 'mn', 0.6, 7, add_legend=True)
myplot(date, 'sm', 1.5, 6, add_legend=False)
myplot(date, 'sn', 1.5, 6, add_legend=False)
myplot(date, 'sr', 4.5, 10, add_legend=False)
myplot(date, 'hj', 2.5, 6, add_legend=False)